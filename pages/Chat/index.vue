...
const myArray = ref(storedArray);

// 监听 myArray 变化，并将其保存到 localStorage 中
watchEffect(() => {
  localStorage.setItem('myArray', JSON.stringify(myArray.value));
});
// 清空 myArray
const clearArray = () => {
  myArray.value = [];
...
useHead({
    title: `${conversationTitle.value} - ChatAI`,
    meta: [
        {
            name: 'viewport',
    ...
    <div class="flex flex-row min-h-screen text-slate-300">
        ...
        >
            <header class="px-3 py-6 text-center bg-purple-500/[15%] shadow relative">
                ...
                <!-- Title -->
<!-- HTML代码 -->
            <h1 class="text-2xl font-bold text-white/80 drop-shadow-md">Chat<span class="font-light">AI</span></h1>
            <button style="float: right;" @click="clearArray">Clear Chat</button>
